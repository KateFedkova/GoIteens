...
class Cash_Machine:
    ...
    def __init__(self, quantity_of_money):
...
Оберіть: """)
    ...
    def info_logger(new_func):
        import logging

        logging.basicConfig(filename=f'{new_func.__name__}.log', level=logging.INFO)
    ...
    @info_logger
    def return_info(self, new_balance):
...
"""
    ...
    @time_logger
    def collection(self):
        code = input('Введіть код для вилучення грошей з банкомату інкасаторами: ')
...
while True:
    ...
    account_1._BankAccount__append_info()
    if place == '1':
